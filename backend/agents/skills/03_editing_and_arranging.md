...
5. Call `update-entity-position` EXACTLY ONCE with the `updates` array containing all positional updates to move them simultaneously.
6. Summarize the new layout.

To delete unused or unwanted devices, use `remove-entity`. Pass `entityIDs` array to delete multiple devices in one call.
